...
function mountScrollWorld(container, config) {
  ...
  function loadClip(s) {
    ...
    // Build the <video> and wire it up. `src` is either a blob: object URL (normal
    // path — always fully seekable regardless of host byte-range support) or the
    // plain file URL (file:// fallback below).
    const attach = (src) => {
    ...
    // file:// pages can't fetch() sibling files (opaque origin), but a <video src>
    // pointing at a local file still loads AND seeks fine. So skip the blob path
    // there — this is what makes double-clicking index.html work.
    if (location.protocol === 'file:') { attach(url); return; }

    fetch(url).then(r => r.ok ? r.blob() : Promise.reject(new Error('404')))
      .then(blob => attach(URL.createObjectURL(blob)))
      .catch(() => { try { attach(url); } catch (e) { s.loading = false; } });
  }
